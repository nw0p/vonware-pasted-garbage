...
public class StringButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            Vonware.textManager.drawStringWithShadow(this.currentString.getString() + Vonware.textManager.getIdleSign(), this.x + 2.3f, this.y - 1.7f - (float) VonwareGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            Vonware.textManager.drawStringWithShadow((this.setting.getName().equals("Buttons") ? "Buttons " : (this.setting.getName().equals("Prefix") ? "Prefix  " + ChatFormatting.GRAY : "")) + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) VonwareGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
