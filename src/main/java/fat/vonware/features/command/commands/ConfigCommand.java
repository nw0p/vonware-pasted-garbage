...
public class ConfigCommand extends Command {
    ...
    public void execute(String[] commands) {
        if (commands.length == 1) {
            sendMessage("You`ll find the config files in your gameProfile directory under vonware/config");
            return;
        }
        if (commands.length == 2)
            if ("list".equals(commands[0])) {
                String configs = "Configs: ";
                File file = new File("vonware/");
        ...
        if (commands.length >= 3) {
            switch (commands[0]) {
                case "save":
                    Vonware.configManager.saveConfig(commands[1]);
                    sendMessage(ChatFormatting.GREEN + "Config '" + commands[1] + "' has been saved.");
                    return;
                case "load":
                    if (Vonware.configManager.configExists(commands[1])) {
                        Vonware.configManager.loadConfig(commands[1]);
