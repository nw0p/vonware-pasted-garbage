...
import fat.vonware.features.setting.EnumConverter;
...
public class ConfigManager implements Util {
    ...
    public String config = "vonware/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        Vonware.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Vonware.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                continue;
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("vonware").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("vonware/" + name + "/"))) {
            this.config = "vonware/" + name + "/";
        } else {
            this.config = "vonware/config/";
        }
        Vonware.friendManager.onLoad();
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("vonware").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("vonware/" + name + "/"));
    }
    ...
    public void saveConfig(String name) {
        ...
        if (!path.exists())
            path.mkdir();
        Vonware.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("vonware/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("vonware", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(Vonware.moduleManager.modules);
        this.features.add(Vonware.friendManager);
        String name = loadCurrentConfig();
        loadConfig(name);
        Vonware.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            Vonware.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
