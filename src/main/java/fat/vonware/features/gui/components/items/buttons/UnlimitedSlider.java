...
public class UnlimitedSlider
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        RenderUtil.drawRect(this.x, this.y, this.x + (float) this.width + 7.4f, this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? Vonware.colorManager.getColorWithAlpha(Vonware.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : Vonware.colorManager.getColorWithAlpha(Vonware.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        Vonware.textManager.drawStringWithShadow(" - " + this.setting.getName() + " " + ChatFormatting.GRAY + this.setting.getValue() + ChatFormatting.WHITE + " +", this.x + 2.3f, this.y - 1.7f - (float) VonwareGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
    }
