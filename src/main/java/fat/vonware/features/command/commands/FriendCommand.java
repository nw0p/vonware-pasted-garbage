...
import fat.vonware.manager.FriendManager;
    ...
    public void execute(String[] commands) {
        if (commands.length == 1) {
            if (Vonware.friendManager.getFriends().isEmpty()) {
                ...
                String f = "Friends: ";
                for (FriendManager.Friend friend : Vonware.friendManager.getFriends()) {
                    try {
        ...
        if (commands.length == 2) {
            ...
            FriendCommand.sendMessage(commands[0] + (Vonware.friendManager.isFriend(commands[0]) ? " is friended." : " isn't friended."));
            return;
        }
        if (commands.length >= 2) {
            switch (commands[0]) {
                case "add": {
                    Vonware.friendManager.addFriend(commands[1]);
                    FriendCommand.sendMessage(ChatFormatting.GREEN + commands[1] + " has been friended");
                    return;
                }
                case "del": {
                    Vonware.friendManager.removeFriend(commands[1]);
