...
public class Slider
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y, ((Number) this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? Vonware.colorManager.getColorWithAlpha(Vonware.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : Vonware.colorManager.getColorWithAlpha(Vonware.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        Vonware.textManager.drawStringWithShadow(this.getName() + " " + ChatFormatting.GRAY + (this.setting.getValue() instanceof Float ? this.setting.getValue() : Double.valueOf(((Number) this.setting.getValue()).doubleValue())), this.x + 2.3f, this.y - 1.7f - (float) VonwareGui.getClickGui().getTextOffset(), -1);
    }
